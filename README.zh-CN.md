...
> **项目状态**：`feature/3.0.x` 版本线早期开发中。制品尚未发布到 Maven Central，通过项目私服与 GitHub Releases 分发。
...
- 不是 HTTP 客户端。`feature/3.0.x` 分支仅提供模型层；OkHttp 虽已声明为依赖，但本分支尚未实现传输层。
...
本分支快照版本为 `3.0.x.x.20260630-SNAPSHOT`。正式版本通过 GitHub Releases 发布；Maven Central 发布已规划，尚未执行。
